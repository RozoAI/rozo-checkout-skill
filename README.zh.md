...
| Solana | `900` | USDC, USDT | 6 位小数；SPL。原生 SOL **不**支持 |
| Stellar | `1500` | USDC | 7 位小数；必须带 memo —— 会在充值信息块里给出 |
...
# 第 4a 步 —— 模式 A（默认）：用任意钱包按充值信息块付款。
#             不需要私钥，也不需要任何配置。然后观察它结清。
node scripts/dist/status.js --rozo-payment-id <uuid> --watch --timeout 600

# 第 4b 步 —— 模式 B（可选）：让脚本替你签名。这是唯一需要私钥的一步。
#             --dry-run 不会签任何名；真正发送还额外需要 --send。
...
- **完整的充值指令。** 金额为零、为负或无法解析都会中止。闪电网络必须有 BOLT11（它随 `source.lnInvoice` 返回，同时地址为空）。一笔 Stellar 充值是一个共享 hub 地址加上一个按订单生成的 memo，所以 memo 是目的地的组成部分：订单若拿不到 memo 即硬性中止，绝不会被渲染成"无需 memo"。
...
- **热钱包管控。** 私钥只从环境变量读取（`ROZO_CHECKOUT_EVM_KEY`、`ROZO_CHECKOUT_SOL_KEY`），永不打印，也绝不接受从命令行传入；库和 RPC 的错误在展示前会被脱敏，包括带凭据的 provider URL、bearer token 和形似密钥的字符串。当工作目录下任何 `.env`/`.env.*` 被 git 跟踪时，脚本拒绝运行；当 git 无法证明它未被跟踪时，同样坚决拒绝。签名前会校验 RPC 的链 id（Solana 则是 genesis hash）以及代币的链上小数位。只有一条上限：单笔付款不得超过 $1,100。没有任何绕过用的开关，也没有单会话累计上限 —— 金额更大的账单就用你自己的钱包去付，那既不需要私钥，也没有上限。
- **地址打码。** 正文中显示为 `first6...last4`。完整的充值地址、memo 和 BOLT11 字符串只出现在机器可读的 `deposit` 对象里，这样它们既保持可复制粘贴，又不会散落在各处日志中。

## 更新日志

- **0.1.1** —— 两条额度上限合并为一条：单笔付款不得超过 $1,100（按购买 $1,000 额度
  加 5% 手续费来设定），单会话累计上限与 `--yes-large` 绕过开关均已移除。文档明确
  说明：用你自己的钱包付款不需要私钥，也不需要任何配置。
