...
| Solana | `900` | USDC, USDT | 小数点以下 6 桁。SPL。ネイティブの SOL は**非対応** |
| Stellar | `1500` | USDC | 小数点以下 7 桁。メモ (memo) が必須 — deposit ブロックに表示されます |
...
# ステップ 3 — 金額、チェーン、マスクされたアドレス、有効期限を確認します。
...
# ステップ 4a — モード A（既定）: deposit ブロックの内容を任意のウォレットから支払います。
#              鍵も設定も不要です。その後の決済完了を監視します。
node scripts/dist/status.js --rozo-payment-id <uuid> --watch --timeout 600

# ステップ 4b — モード B（任意）: スクリプトに署名させます。秘密鍵が必要なのはこの
#              ステップだけです。--dry-run では署名は一切行いません。実際に送金する
#              にはさらに --send が必要です。
...
- **完全な入金手順。** 金額がゼロ、負数、あるいは解析不能な場合は中断します。Lightning では
  BOLT11 が必須です（アドレスは空で、`source.lnInvoice` に入って届きます）。Stellar の入金は
  共有ハブアドレスと注文ごとのメモの組み合わせであり、メモは宛先の一部です。したがって
  メモを伴わずに届いた注文は強制中断となり、「メモ不要」と表示されることは決してありません。
...
- **ホットウォレットの制御。** 鍵は環境変数からのみ取得され
  ...
  適用される上限は 1 つだけです。1 回の支払いは $1,100 を超えられません。上書き用の
  フラグはなく、セッション累計の上限もありません — これを超えるインボイスは自分の
  ウォレットから支払ってください。そちらは鍵が不要で、上限もありません。
